package com.example.client.contact;
...
import com.example.client.common.http.UcstarHttpClient;
import com.example.client.utils.log.LogUtil;
import com.example.client.utils.string.MD5;
import com.example.client.config.DemoServers;
...
public class ContactHttpClient {
    ...
    private static com.example.client.contact.ContactHttpClient instance;

    public static synchronized com.example.client.contact.ContactHttpClient getInstance() {
        if (instance == null) {
            instance = new com.example.client.contact.ContactHttpClient();
        }
    ...
    private ContactHttpClient() {
        UcstarHttpClient.getInstance().init(SDKGlobal.getContext());
    }


    /**
     * 向应用服务器创建账号（注册账号）
     * 由应用服务器调用WEB SDK接口将新注册的用户数据同步到UcSTAR服务器
     */
    public void register(String account, String nickName, String password, final ContactHttpCallback<Void> callback) {
        ...
        UcstarHttpClient.getInstance().execute(url, headers, bodyString, new UcstarHttpClient.UcstarHttpCallback() {
            @Override
    ...
    private String readAppKey() {
        try {
            ApplicationInfo appInfo = SDKGlobal.getContext().getPackageManager()
                    .getApplicationInfo(SDKGlobal.getContext().getPackageName(), PackageManager.GET_META_DATA);
            if (appInfo != null) {
                return appInfo.metaData.getString("com.shenhua.zhihui.appKey");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
